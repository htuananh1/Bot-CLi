...
def process_with_gemini(prompt, input_path, output_path):
    ...
    cmd = ["gemini", "ask", f"{prompt} + tài liệu trong file", "--file", input_path, "--output", output_path]
...
async def handle_file(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
        ...
        reply = result.stdout.strip() if result.stdout else result.stderr.strip()
        await update.message.reply_text("Không tạo được file.\n" + reply[:4000])
